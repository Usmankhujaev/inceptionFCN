...
from utils.constants import SMALL_DATASET as TSC10
...
def read_all_datasets(root_dir, archive_name):
    ...
    if archive_name == 'TSC':
    ...
    elif archive_name == 'TSC10':
        for dataset_name in TSC10:
            root_dir_dataset = root_dir + '/archives/' + archive_name + '/' + dataset_name + '/'
            file_name = root_dir_dataset + dataset_name
            x_train, y_train = readucr(file_name + '_TRAIN')
            x_test, y_test = readucr(file_name + '_TEST')
        ...
        for i in range(len(TSC10)):
            TSC10[i] = dataset_names_to_sort[i][0]
